...
## [Unreleased]
- Synced repository docs for release defaults: clarified `DEBUG_ENDPOINTS` and `VENORA_AUTH_DEBUG` as `false` by default in README and architecture notes
- Synced debug gating snippet in `docs/EDGE_CASE_FIXES.md` to match backend (`DEBUG_ENDPOINTS` default `false`)
- Normalized dashboard banner kicker text to `Hey there...!` (first-letter uppercase only)
- Replaced Practice phrase number input with a phrase selector (`Phrase 1..N`, final alankar option labeled `Combined Phrase`)
