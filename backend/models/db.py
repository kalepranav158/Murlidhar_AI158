...
DB_NAME = str(_PROJECT_ROOT / "data" / "db" / "practice_data.db")
_TEST_DB_FILE = None
...
def save_session(user_id, reference, played, result, skill_id: str | None = None, testng=False):
    if not testng:
        ...
        # ðŸ”’ BEGIN IMMEDIATE TRANSACTION
        cursor.execute("BEGIN IMMEDIATE")

        # 1ï¸âƒ£ Compute deterministic session hash
...
def count_mastered_songs(user_id: str) -> int:
    ...
    for song_id, mastered_phrases in mastered_by_song.items():
        try:
            song = load_song(f"data/songs/catalog/{song_id}.json")
