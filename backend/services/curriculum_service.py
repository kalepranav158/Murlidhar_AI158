...
from backend.models.db import get_student_progress  # type: ignore[attr-defined]
from backend.models.db import update_student_progress  # type: ignore[attr-defined]
from backend.models.db import is_alankar_mastered  # type: ignore[attr-defined]
from backend.models.db import is_melody_mastered  # type: ignore[attr-defined]
from backend.models.db import is_song_mastered  # type: ignore[attr-defined]
from backend.models.db import count_mastered_alankars  # type: ignore[attr-defined]
from backend.models.db import count_mastered_songs  # type: ignore[attr-defined]
...
def evaluate_curriculum_progress(user_id: str) -> dict:
    ...
    if not unlocked:
        for fname in glob.glob("data/songs/catalog/*.json"):
            try:
    ...
    for cid in list(unlocked):
        if cid in mastered:
            continue
        path = f"data/songs/catalog/{cid}.json"
