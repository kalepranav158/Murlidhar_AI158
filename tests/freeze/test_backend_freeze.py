...
from backend.services.adaptive_engine import build_snapshot
...
def _seed_curriculum_files(skill_id: str, next_skill_id: str):
    with open(f"data/songs/catalog/{skill_id}.json", "w", encoding="utf-8") as handle:
        json.dump(
            {
                "id": skill_id,
                "type": "alankar",
    ...
    with open(f"data/songs/catalog/{next_skill_id}.json", "w", encoding="utf-8") as handle:
        json.dump(
            {
                "id": next_skill_id,
                "type": "alankar",
...
def test_freeze_full_integration_chain():
    ...
    try:
        ...
        _remove_curriculum_files(f"data/songs/catalog/{skill_id}.json", f"data/songs/catalog/{next_skill_id}.json")
